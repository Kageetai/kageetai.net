...
changed: 2025-12-02T10:06+01:00
...
![Quartz-based web page](./attachments/new%20blog,%20new%20stack-1763475494080.webp)  
  
Certainly not bad for a basic setup and being able to publish Obsidian notes relatively easily.  
I did keep this for a while, but always with the desire to only use it as a base and at some point replace it with.  
...
- converting Wikilinks into standard Markdown links and map them to the corresponding files: `[[link]]` -> `[](link)` (which was more complexities than I thought at first, e.g. what do to with identically named notes in different folders?)  
...
For that kinda of choice there are just as many options out there. I won't go over those here in detail, basically any frontend framework would do the job. I chose [Astro](https://astro.build/) for my purpose here, as it's suited well for static content like blogs and supports many modern web features and technologies.  
...
For more details about that, I just recommend just checking the source code and history on [GitHub](https://github.com/Kageetai/kageetai.net/commits/main/). I will surely work on it more, try to find my own style and design etc. For now I want to keep it simple, but have plans for more proper "blog features" and such.  
